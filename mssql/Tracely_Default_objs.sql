-- Check to see if the options are enabled.
SELECT * FROM sys.configurations WHERE configuration_id = 1568
SELECT * FROM ::fn_trace_getinfo(0)

-- Sometimes the option is enabled and it still won't write to the log. Turn Off/Turn On.
...
--	[ObjectTyp] VARCHAR(100)
--) ON [PRIMARY]
--GO

-- Look at some data to see if it's working.
INSERT INTO [DBA_COMMON].dbo.[Obj_log]([name],[subclass_name],[DBName],[NTDomainName],[NTUserName],[HostName],[ApplicationName],[LoginName],[StartTime],[ObjectName],[ObjectTyp])
(SELECT  TE.name ,
        v.subclass_name ,
...
WHERE   TE.name IN ('Object:Created', 'Object:Deleted', 'Object:Altered')
        ...
        AND StartTime > DATEADD(HH, -12, GETDATE())
)

SELECT * FROM [DBA_COMMON].dbo.[Obj_log]
